...
const PCOS_TYPES = {
  insulin: { name: 'Insulin-resistant PCOS', badge: 'Most common type', desc: 'Your body over-produces insulin, signaling your ovaries to make more androgens. This type responds very well to targeted diet and strength training.', symptoms: ['Belly weight gain', 'Sugar cravings', 'Energy crashes', 'Irregular periods'], tip: 'Prioritize protein and fiber at every meal to keep blood sugar stable. Strength training 3x/week significantly improves insulin sensitivity.', color: '#E8F7F1', textColor: '#0F6E56' },
  adrenal: { name: 'Adrenal PCOS', badge: 'Stress-driven type', desc: 'Your adrenal glands over-produce androgens in response to chronic stress. High-intensity exercise can worsen this type.', symptoms: ['High stress', 'Fatigue despite sleep', 'Acne and hair loss', 'Slim but symptomatic'], tip: 'Stress management is your treatment. Never skip meals. Yoga and walking are more healing than intense workouts.', color: '#FEF3E2', textColor: '#7A4F00' },
  inflammatory: { name: 'Inflammatory PCOS', badge: 'Gut & immune-driven', desc: 'Chronic low-grade inflammation is triggering your ovaries to overproduce androgens. Healing your gut is the most powerful lever.', symptoms: ['Bloating and gut issues', 'Brain fog', 'Joint pain', 'Food sensitivities'], tip: 'Try eliminating gluten and dairy for 6 weeks. Load up on omega-3s, turmeric, and fermented foods.', color: '#E8F0FB', textColor: '#1A4F8A' },
  postpill: { name: 'Post-pill PCOS', badge: 'Temporary & reversible', desc: 'Your hormones are recalibrating after stopping birth control. This type often resolves in 3-6 months with the right support.', symptoms: ['Irregular after stopping pill', 'Acne flare-up', 'Normal weight', 'Previously regular cycle'], tip: 'Support your liver with cruciferous vegetables, zinc-rich foods, and B vitamins. Try seed cycling.', color: '#F7D6DF', textColor: '#6B2438' }
};
...
export default function App() {
  ...
  const pk = '#C4687E'; const pkl = '#FDF0F3'; const pkm = '#F7D6DF'; const pkb = '#F0B8C8'; const pkd = '#6B2438'; const pkt = '#8B3A50';
  ...
  async function loadProfile(u) {
    ...
    if (data) {
      ...
      setPcosType(data.pcos_type || 'insulin');
      setPhase(data.phase || 'follicular');
      setMealSetup(prev => ({ ...prev, type: data.pcos_type || 'insulin', phase: data.phase || 'follicular' }));
      setWorkoutSetup(prev => ({ ...prev, type: data.pcos_type || 'insulin', phase: data.phase || 'follicular' }));
    }
    const { data: logsData } = await supabase.from('logs').select('*').eq('user_id', u.id).order('created_at', { ascending: false }).limit(10);
    if (logsData) setLogs(logsData.map(l => ({ ...l, symptoms: JSON.parse(l.symptoms || '[]') })));
  }
  ...
  async function signUp() {
    ...
    if (data.user) {
      await supabase.from('profiles').insert({ id: data.user.id, email: authEmail, name: authName, pcos_type: 'insulin', phase: 'follicular', goal: 'lose weight' });
  ...
  async function signOut() {
    ...
    setUser(null); setUserName(''); setPage('home');
  }
  ...
  function selectPhase(p) {
    ...
    if (user) saveProfile({ phase: p });
  }

  function toggleRestriction(item) {
    setMealSetup(prev => ({ ...prev, restrictions: prev.restrictions.includes(item) ? prev.restrictions.filter(r => r !== item) : [...prev.restrictions, item] }));
  }

  function toggleEquipment(item) {
    setWorkoutSetup(prev => ({ ...prev, equipment: prev.equipment.includes(item) ? prev.equipment.filter(e => e !== item) : [...prev.equipment, item] }));
  }

  function selectQ(i) {
    const id = QUESTIONS[qCurrent].id;
    setAnswers(prev => ({ ...prev, [id]: i }));
    setTimeout(() => { if (qCurrent < QUESTIONS.length - 1) setQCurrent(c => c + 1); else finishQuiz({ ...answers, [id]: i }); }, 280);
  }

  function toggleMulti(i) {
    const id = QUESTIONS[qCurrent].id;
    setMulti(prev => { const arr = prev[id] || []; return { ...prev, [id]: arr.includes(i) ? arr.filter(x => x !== i) : [...arr, i] }; });
  }
  ...
  function finishQuiz(finalAnswers) {
    ...
    if (name) { setUserName(name); if (user) saveProfile({ name, pcos_type: type }); }
    setMealSetup(prev => ({ ...prev, type }));
    setWorkoutSetup(prev => ({ ...prev, type }));
    setQuizStep('result');
  }

  function resetQuiz() { setQCurrent(0); setAnswers({}); setMulti({}); setQuizStep('intro'); }
  ...
  async function callAI(prompt, onSuccess, onError) {
    ...
    try {
      const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${OPENROUTER_KEY}`, 'HTTP-Referer': 'https://hormonize.fit', 'X-Title': 'Hormonize' },
        body: JSON.stringify({ model: 'mistralai/mistral-nemo', max_tokens: 4000, messages: [{ role: 'user', content: prompt }] })
      });
  ...
  async function genWorkouts() {
    ...
    const prompt = `You are a certified personal trainer specializing in PCOS. Create a 7-day cycle-synced workout plan for: PCOS type: ${type}, Cycle phase: ${p}, Fitness: ${fitness}, Duration: ${duration}, Goal: ${goal}, Equipment: ${equipment.join(', ')}. Return ONLY valid JSON no markdown: {"title":"","subtitle":"","phase_tip":"","active_days":0,"rest_days":0,"avg_duration":"","days":[{"day":"Monday","is_rest":false,"workout_type":"","intensity":"low","name":"","description":"2 sentences","exercises":[{"name":"","sets_reps":"","muscle_group":""}],"duration_min":0,"cooldown":""}]}. 7 days Monday–Sunday. Rest days is_rest:true exercises:[]. Min 1-2 rest days.`;
  ...
  async function saveLog() {
    ...
    const newLog = { ...todayLog, date: today };
    if (user) {
      await supabase.from('logs').insert({ user_id: user.id, date: today, mood: todayLog.mood, energy: todayLog.energy, bloating: todayLog.bloating, symptoms: JSON.stringify(todayLog.symptoms), notes: todayLog.notes });
    }
...
async function handleUpgrade(planKey) {
  ...
  if (!user) {
    return (
      <div style={{ maxWidth: 480, margin: '0 auto', minHeight: '100vh', background: pkl, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1.25rem', fontFamily: "'Inter', system-ui, sans-serif" }}>
        <div style={{ width: '100%' }}>
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            <div style={{ fontSize: 48, marginBottom: '0.75rem' }}>🌸</div>
            <div style={{ fontSize: 28, fontWeight: 700, color: pkd, letterSpacing: '-0.02em' }}>Hormonize</div>
            <div style={{ fontSize: 14, color: pkt, marginTop: 4 }}>Your PCOS wellness companion</div>
          </div>
          <div style={{ background: '#fff', borderRadius: 24, padding: '2rem', border: `0.5px solid ${pkb}`, boxShadow: '0 8px 32px rgba(196,104,126,0.1)' }}>
            <div style={{ display: 'flex', background: pkm, borderRadius: 16, padding: 4, marginBottom: '1.5rem' }}>
              {[['login', 'Sign in'], ['signup', 'Create account']].map(([s, l]) => (
                <button key={s} onClick={() => { setAuthScreen(s); setAuthError(''); }} style={{ flex: 1, padding: '9px', borderRadius: 12, border: authScreen === s ? `0.5px solid ${pkb}` : 'none', background: authScreen === s ? '#fff' : 'none', fontSize: 14, cursor: 'pointer', color: pkd, fontWeight: authScreen === s ? 600 : 400, fontFamily: 'inherit' }}>{l}</button>
              ))}
            </div>
            {authError && <div style={{ background: '#FCEBEB', border: '0.5px solid #F4BEBE', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#A32D2D', marginBottom: '1rem' }}>{authError}</div>}
            {authScreen === 'signup' && (
              <div style={{ marginBottom: '1rem' }}>
                <label style={{ fontSize: 13, fontWeight: 500, color: pkt, marginBottom: 6, display: 'block' }}>Your name</label>
                <input type="text" value={authName} onChange={e => setAuthName(e.target.value)} placeholder="First name" style={{ width: '100%', padding: '12px 14px', border: `0.5px solid ${pkb}`, borderRadius: 12, fontSize: 14, fontFamily: 'inherit', outline: 'none' }} />
              </div>
            )}
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: pkt, marginBottom: 6, display: 'block' }}>Email</label>
              <input type="email" value={authEmail} onChange={e => setAuthEmail(e.target.value)} placeholder="your@email.com" style={{ width: '100%', padding: '12px 14px', border: `0.5px solid ${pkb}`, borderRadius: 12, fontSize: 14, fontFamily: 'inherit', outline: 'none' }} />
            </div>
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ fontSize: 13, fontWeight: 500, color: pkt, marginBottom: 6, display: 'block' }}>Password</label>
              <input type="password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} placeholder="At least 6 characters" style={{ width: '100%', padding: '12px 14px', border: `0.5px solid ${pkb}`, borderRadius: 12, fontSize: 14, fontFamily: 'inherit', outline: 'none' }} />
            </div>
            <button onClick={authScreen === 'login' ? signIn : signUp} disabled={authLoading} style={{ width: '100%', padding: '14px', background: authLoading ? pkb : pk, border: 'none', borderRadius: 14, color: '#fff', fontSize: 15, fontWeight: 600, cursor: authLoading ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
              {authLoading ? 'Please wait…' : authScreen === 'login' ? 'Sign in →' : 'Create account →'}
            </button>
            <p style={{ fontSize: 12, color: '#aaa', textAlign: 'center', marginTop: '1rem', lineHeight: 1.6 }}>Educational use only · Not medical advice</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', background: pkl, minHeight: '100vh', fontFamily: "'Inter', system-ui, sans-serif" }}>
      <style>{`
        ...
        select,input[type=text],input[type=email],input[type=password],textarea{font-family:inherit;width:100%;padding:11px 14px;border:0.5px solid ${pkb};border-radius:12px;font-size:14px;background:#fff;color:#111;outline:none}
        ...
        .day-tab{flex-shrink:0;padding:7px 16px;border-radius:20px;font-size:13px;cursor:pointer;background:#fff;color:${pkt};font-family:inherit;border:0.5px solid ${pkb}}
        .day-tab.active{background:${pk};color:#fff;font-weight:500;border-color:${pk}}
      ...
      <div style={{ background: '#fff', borderBottom: `0.5px solid ${pkb}`, padding: '0 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56, position: 'sticky', top: 0, zIndex: 20 }}>
        <div style={{ fontSize: 20, fontWeight: 700, color: pk, display: 'flex', alignItems: 'center', gap: 8, letterSpacing: '-0.02em' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 500, background: pkm, color: pkd, border: `0.5px solid ${pkb}` }}>{plan === 'pro' ? '✨ Pro' : 'Free'}</div>
          <div onClick={() => goPage('profile')} style={{ width: 32, height: 32, borderRadius: '50%', background: `linear-gradient(135deg,${pk},${pkd})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: '#fff', cursor: 'pointer' }}>{userName ? userName.charAt(0).toUpperCase() : '🌸'}</div>
        </div>
      </div>
      ...
      <div style={{ padding: '1.25rem', paddingBottom: 90 }}>
        ...
        {page === 'home' && (
          <div className="fade-in">
            <div style={{ background: `linear-gradient(135deg,${pkm},#FDF0F3,#F7E8EF)`, border: `0.5px solid ${pkb}`, borderRadius: 24, padding: '2rem 1.5rem', marginBottom: '1.5rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', top: -20, right: -20, width: 120, height: 120, borderRadius: '50%', background: 'rgba(196,104,126,0.08)' }} />
              <div style={{ fontSize: 48, marginBottom: '0.75rem' }}>🌸</div>
              <div style={{ fontSize: 26, fontWeight: 700, color: pkd, marginBottom: '0.5rem', letterSpacing: '-0.02em' }}>Welcome back{userName ? `, ${userName}` : ''}!</div>
              <div style={{ fontSize: 15, color: pkt, lineHeight: 1.7, maxWidth: 360, margin: '0 auto 1.5rem' }}>Your personalized PCOS wellness companion.</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: '1.5rem' }}>
              {[{ key: 'follicular', name: '🌱 Follicular', days: 'Day 1–13', tip: 'Rising energy. Great for strength.', bg: '#E8F7F1', color: '#0F6E56' }, { key: 'ovulatory', name: '🌸 Ovulatory', days: 'Day 14–16', tip: 'Peak performance window.', bg: pkm, color: pkd }, { key: 'luteal', name: '🍂 Luteal', days: 'Day 17–28', tip: 'Slow down and prioritize rest.', bg: '#FEF3E2', color: '#7A4F00' }, { key: 'menstrual', name: '🌙 Menstrual', days: 'Period', tip: 'Rest deeply. Gentle movement.', bg: '#E8F0FB', color: '#1A4F8A' }].map(ph => (
                <div key={ph.key} onClick={() => selectPhase(ph.key)} style={{ padding: '14px', borderRadius: 16, border: phase === ph.key ? `2px solid ${pk}` : '0.5px solid transparent', background: ph.bg, cursor: 'pointer', boxShadow: phase === ph.key ? `0 4px 12px rgba(196,104,126,0.2)` : 'none' }}>
            ...
            {[{ label: '🧬 PCOS assessment', sub: 'Identify your type in 3 minutes', page: 'quiz' }, { label: '🥗 Meal plan generator', sub: '7-day hormone-synced nutrition with photos', page: 'meals' }, { label: '💪 Workout planner', sub: 'Cycle-synced exercise with diagrams', page: 'workouts' }, { label: '📊 Daily tracker', sub: 'Log symptoms, mood, and energy', page: 'tracker' }].map(item => (
              <div key={item.page} onClick={() => goPage(item.page)} className="card" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div><div style={{ fontSize: 15, fontWeight: 500 }}>{item.label}</div><div style={{ fontSize: 13, color: '#888', marginTop: 3 }}>{item.sub}</div></div>
          ...
          <div className="fade-in">
            {quizStep === 'intro' && (
              <div style={{ textAlign: 'center', padding: '2rem 0' }}>
                ...
                <div style={{ fontSize: 24, fontWeight: 700, color: pkd, marginBottom: '0.5rem' }}>PCOS assessment</div>
            ...
            {quizStep === 'q' && (
              <div>
                ...
                {q.type === 'text' && <input type="text" value={answers[q.id] || ''} onChange={e => setAnswers(prev => ({ ...prev, [q.id]: e.target.value }))} placeholder={q.placeholder} style={{ marginBottom: '1rem' }} />}
                {q.type === 'single' && q.opts.map((o, i) => (
                  <button key={i} onClick={() => selectQ(i)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', border: answers[q.id] === i ? `1.5px solid ${pk}` : `0.5px solid ${pkb}`, borderRadius: 14, background: answers[q.id] === i ? pkm : '#fff', cursor: 'pointer', textAlign: 'left', width: '100%', marginBottom: 8, fontSize: 14, fontFamily: 'inherit' }}>
                    <div style={{ width: 18, height: 18, borderRadius: '50%', border: answers[q.id] === i ? `5px solid ${pk}` : `1.5px solid ${pkb}`, flexShrink: 0 }} />
              ...
              return (
                <div className="fade-in">
                  <div style={{ textAlign: 'center', padding: '1rem 0 1.5rem' }}>
                    ...
                    <div style={{ fontSize: 22, fontWeight: 700, marginBottom: '0.5rem', color: '#111' }}>{userName ? `${userName}, you have ` : 'You have '}{info.name}</div>
        ...
        {page === 'meals' && (
          <div className="fade-in">
            {mealScreen === 'setup' && (
              <div>
                <div style={{ fontSize: 22, fontWeight: 700, marginBottom: '0.3rem', color: '#111' }}>🥗 Meal plan generator</div>
                <div style={{ fontSize: 14, color: '#888', marginBottom: '1.5rem' }}>A full 7-day plan with photos, built for your PCOS type.</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {[{ label: 'PCOS type', key: 'type', opts: [['insulin', 'Insulin-resistant'], ['adrenal', 'Adrenal'], ['inflammatory', 'Inflammatory'], ['postpill', 'Post-pill']] }, { label: 'Goal', key: 'goal', opts: [['lose weight', 'Lose weight'], ['gain weight', 'Gain weight'], ['maintain weight', 'Maintain'], ['balance hormones', 'Balance hormones']] }, { label: 'Cycle phase', key: 'phase', opts: [['follicular', 'Follicular'], ['ovulatory', 'Ovulatory'], ['luteal', 'Luteal'], ['menstrual', 'Menstrual']] }, { label: 'Meals per day', key: 'meals', opts: [['3', '3 meals'], ['3 meals and 1 snack', '3 + snack'], ['3 meals and 2 snacks', '3 + 2 snacks'], ['5 small meals', '5 small meals']] }].map(f => (
                    <div key={f.key} style={{ marginBottom: '1rem' }}>
                      ...
                      <select value={mealSetup[f.key]} onChange={e => setMealSetup(prev => ({ ...prev, [f.key]: e.target.value }))}>{f.opts.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select>
                    </div>
                  ))}
                </div>
                <label style={{ fontSize: 13, fontWeight: 500, color: pkt, marginBottom: 8, display: 'block' }}>Dietary restrictions</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: '1.25rem' }}>
                  {['Gluten-free', 'Dairy-free', 'Vegetarian', 'Vegan', 'Low-carb', 'Nut-free'].map(r => <button key={r} className={`chip ${mealSetup.restrictions.includes(r) ? 'on' : ''}`} onClick={() => toggleRestriction(r)}>{r}</button>)}
                </div>
            ...
            {mealScreen === 'loading' && <div style={{ textAlign: 'center', padding: '4rem 1rem' }}><div style={{ width: 48, height: 48, border: `3px solid ${pkb}`, borderTopColor: pk, borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 1.25rem' }} /><div style={{ fontSize: 15, color: pkt, fontWeight: 500 }}>{loadingMsg}</div></div>}
            {mealScreen === 'result' && mealPlan && (
              <div className="fade-in">
                ...
                <div style={{ fontSize: 20, fontWeight: 700, marginBottom: '0.25rem' }}>{mealPlan.title}</div>
                <div style={{ fontSize: 13, color: '#888', marginBottom: '1.25rem' }}>{mealPlan.subtitle}</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: '1.25rem' }}>
                  {[{ val: Math.round(mealPlan.daily_calories), lbl: 'Cal/day', icon: '🔥' }, { val: Math.round(mealPlan.protein_g) + 'g', lbl: 'Protein', icon: '💪' }, { val: Math.round(mealPlan.carbs_g) + 'g', lbl: 'Carbs', icon: '🌾' }].map(s => (
                    <div key={s.lbl} style={{ background: '#fff', border: `0.5px solid ${pkb}`, borderRadius: 14, padding: '12px 10px', textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: 20, fontWeight: 700, color: pk }}>{s.val}</div>
                ...
                <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 6, marginBottom: '1.25rem' }}>
                  {mealPlan.days.map((d, i) => <button key={i} className={`day-tab ${mealDay === i ? 'active' : ''}`} onClick={() => setMealDay(i)}>{d.day.slice(0, 3)}</button>)}
                </div>
                {mealPlan.days[mealDay].meals.map((m, i) => (
                  <div key={i} style={{ background: '#fff', border: `0.5px solid ${pkb}`, borderRadius: 16, marginBottom: 14, overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '1rem 1.25rem' }}>
                      ...
                      <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 6 }}>{m.name}</div>
                      <div style={{ fontSize: 13, color: '#666', lineHeight: 1.7, marginBottom: 10 }}>{m.description}</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>{(m.tags || []).map(t => <span key={t} style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, background: pkl, border: `0.5px solid ${pkb}`, color: pkt, fontWeight: 500 }}>{t}</span>)}</div>
                    </div>
                  </div>
                ))}
        ...
        {page === 'workouts' && (
          <div className="fade-in">
            {workoutScreen === 'setup' && (
              <div>
                <div style={{ fontSize: 22, fontWeight: 700, marginBottom: '0.3rem' }}>💪 Workout planner</div>
                <div style={{ fontSize: 14, color: '#888', marginBottom: '1.5rem' }}>A cycle-synced 7-day workout schedule with exercise diagrams.</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {[{ label: 'PCOS type', key: 'type', opts: [['insulin', 'Insulin-resistant'], ['adrenal', 'Adrenal'], ['inflammatory', 'Inflammatory'], ['postpill', 'Post-pill']] }, { label: 'Cycle phase', key: 'phase', opts: [['follicular', 'Follicular'], ['ovulatory', 'Ovulatory'], ['luteal', 'Luteal'], ['menstrual', 'Menstrual']] }, { label: 'Fitness level', key: 'fitness', opts: [['beginner', 'Beginner'], ['intermediate', 'Intermediate'], ['advanced', 'Advanced']] }, { label: 'Session length', key: 'duration', opts: [['20–30 minutes', '20–30 min'], ['30–45 minutes', '30–45 min'], ['45–60 minutes', '45–60 min']] }, { label: 'Goal', key: 'goal', opts: [['lose fat', 'Lose fat'], ['build muscle', 'Build muscle'], ['balance hormones', 'Balance hormones'], ['reduce stress', 'Reduce stress']] }].map(f => (
                    <div key={f.key} style={{ marginBottom: '1rem' }}>
                      <label style={{ fontSize: 13, fontWeight: 500, color: pkt, marginBottom: 6, display: 'block' }}>{f.label}</label>
                      <select value={workoutSetup[f.key]} onChange={e => setWorkoutSetup(prev => ({ ...prev, [f.key]: e.target.value }))}>{f.opts.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select>
                    </div>
                  ))}
                </div>
                <label style={{ fontSize: 13, fontWeight: 500, color: pkt, marginBottom: 8, display: 'block' }}>Equipment</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: '1.25rem' }}>
                  {['No equipment', 'Dumbbells', 'Resistance bands', 'Gym access', 'Yoga mat'].map(e => <button key={e} className={`chip ${workoutSetup.equipment.includes(e) ? 'on' : ''}`} onClick={() => toggleEquipment(e)}>{e}</button>)}
                </div>
                <button className="pink-btn" onClick={genWorkouts}>Generate workout plan →</button>
              </div>
            )}
            {workoutScreen === 'loading' && <div style={{ textAlign: 'center', padding: '4rem 1rem' }}><div style={{ width: 48, height: 48, border: `3px solid ${pkb}`, borderTopColor: pk, borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 1.25rem' }} /><div style={{ fontSize: 15, color: pkt, fontWeight: 500 }}>{loadingMsg}</div></div>}
            {workoutScreen === 'result' && workoutPlan && (
              <div className="fade-in">
                ...
                <div style={{ fontSize: 20, fontWeight: 700, marginBottom: '0.25rem' }}>{workoutPlan.title}</div>
                <div style={{ fontSize: 13, color: '#888', marginBottom: '1.25rem' }}>{workoutPlan.subtitle}</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: '1.25rem' }}>
                ...
                <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 6, marginBottom: '1.25rem' }}>
                  {workoutPlan.days.map((d, i) => <button key={i} className={`day-tab ${workoutDay === i ? 'active' : ''}`} onClick={() => setWorkoutDay(i)}>{d.day.slice(0, 3)}</button>)}
                </div>
                {workoutPlan.days[workoutDay].is_rest ? (
                  <div style={{ background: '#fff', border: `0.5px dashed ${pkb}`, borderRadius: 16, padding: '1.5rem', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 17, fontWeight: 600, marginBottom: 6 }}>{workoutPlan.days[workoutDay].name}</div>
                    <div style={{ fontSize: 14, color: '#666', lineHeight: 1.6 }}>{workoutPlan.days[workoutDay].description}</div>
                  </div>
                ) : (
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                      <div style={{ fontSize: 16, fontWeight: 600 }}>{workoutPlan.days[workoutDay].name}</div>
        ...
        {page === 'tracker' && (
          <div className="fade-in">
            <div style={{ fontSize: 22, fontWeight: 700, marginBottom: '0.25rem' }}>📊 Daily tracker</div>
              ...
              <div>
                ...
                {logs.slice(0, 5).map((l, i) => (
                  <div key={i} className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div><div style={{ fontSize: 14, fontWeight: 500 }}>{l.date}</div><div style={{ fontSize: 12, color: pkt, marginTop: 2 }}>{l.mood} · Energy {l.energy}/10</div></div>
                    <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>{(Array.isArray(l.symptoms) ? l.symptoms : []).slice(0, 3).map(s => <span key={s} style={{ padding: '2px 8px', borderRadius: 10, fontSize: 11, background: pkm, color: pkd, fontWeight: 500 }}>{s}</span>)}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        {page === 'subscription' && (
          <div className="fade-in">
            <div style={{ textAlign: 'center', padding: '1.5rem 0 1rem' }}>
              <div style={{ fontSize: 48, marginBottom: '0.75rem' }}>✨</div>
              <div style={{ fontSize: 24, fontWeight: 700, color: pkd, marginBottom: '0.5rem' }}>Upgrade Hormonize</div>
            ...
            {[{ key: 'free', name: 'Free', emoji: '🌱', features: [{ y: true, t: 'PCOS assessment' }, { y: true, t: '1 meal plan/month' }, { y: false, t: 'Unlimited AI plans' }, { y: false, t: 'Saved logs & history' }], btn: 'Current plan', primary: false },
              { key: 'basic', name: 'Basic', emoji: '🌸', features: [{ y: true, t: 'PCOS assessment' }, { y: true, t: '4 meal plans/month' }, { y: true, t: 'Saved logs & history' }, { y: false, t: 'Unlimited AI plans' }], btn: 'Start free trial', primary: false },
              { key: 'pro', name: 'Pro', emoji: '✨', features: [{ y: true, t: 'PCOS assessment' }, { y: true, t: 'Unlimited meal plans' }, { y: true, t: 'Saved logs & history' }, { y: true, t: 'Unlimited AI plans' }], btn: 'Upgrade to Pro', primary: true, popular: true }
            ].map(t => (
            ...
            <div style={{ background: pkm, border: `0.5px solid ${pkb}`, borderRadius: 20, padding: '1.5rem', marginBottom: '1rem' }}>
              <div style={{ fontSize: 16, fontWeight: 700, color: pkd, marginBottom: '1rem' }}>📊 Revenue estimator</div>
        ...
        {page === 'profile' && (
          <div className="fade-in">
            ...
            <div style={{ fontSize: 20, fontWeight: 700, textAlign: 'center', marginBottom: '0.25rem' }}>{userName || 'My profile'}</div>
            <div style={{ fontSize: 13, color: '#888', textAlign: 'center', marginBottom: '0.5rem' }}>{user?.email}</div>
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1.5rem' }}><div style={{ padding: '4px 14px', borderRadius: 20, fontSize: 12, fontWeight: 500, background: pkm, color: pkd, border: `0.5px solid ${pkb}` }}>{plan === 'pro' ? '✨ Pro plan' : '🌱 Free plan'}</div></div>
            <div className="card" style={{ padding: '0 1.25rem', marginBottom: 10 }}>
              {[{ label: '🧬 PCOS assessment', sub: 'Identify your type', action: () => goPage('quiz') }, { label: '📊 Daily tracker', sub: 'View your logs', action: () => goPage('tracker') }, { label: '✨ Upgrade to Pro', sub: 'Unlock all features', action: () => goPage('subscription'), pink: true }].map((item, i) => (
                <div key={i} onClick={item.action} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: i < 2 ? `0.5px solid ${pkb}` : 'none', cursor: 'pointer' }}>
                  <div><div style={{ fontSize: 14, fontWeight: 500, color: item.pink ? pk : '#111' }}>{item.label}</div><div style={{ fontSize: 12, color: '#888', marginTop: 2 }}>{item.sub}</div></div>
                  <span style={{ color: pkb, fontSize: 20 }}>›</span>
                </div>
              ))}
            </div>
            <button onClick={signOut} style={{ width: '100%', padding: 13, background: 'none', border: `0.5px solid ${pkb}`, borderRadius: 14, color: '#888', fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>Sign out</button>
      ...
      <div style={{ background: '#fff', borderTop: `0.5px solid ${pkb}`, display: 'flex', position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 680, zIndex: 20 }}>
        {[{ label: 'Home', p: 'home', icon: '🏠' }, { label: 'Quiz', p: 'quiz', icon: '🧬' }, { label: 'Meals', p: 'meals', icon: '🥗' }, { label: 'Workouts', p: 'workouts', icon: '💪' }, { label: 'Tracker', p: 'tracker', icon: '📊' }, { label: 'Upgrade', p: 'subscription', icon: '✨' }, { label: 'Profile', p: 'profile', icon: '👤' }].map(n => (
